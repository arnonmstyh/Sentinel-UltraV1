...
const SEVERITY_META: { key: string; label: string; color: string; glow: string }[] = [
  { key: "critical", label: "Critical", color: "#ef4444", glow: "#ef444460" },
  { key: "high",     label: "High",     color: "#f97316", glow: "#f9731660" },
  { key: "medium",   label: "Medium",   color: "#f59e0b", glow: "#f59e0b60" },
  { key: "low",      label: "Low",      color: "#22c55e", glow: "#22c55e60" },
];
...
function describeArc(
  ...
  // Clamp to avoid full-circle SVG arc bug
...
const SeverityDistribution = () => {
  ...
  const { arcs, total } = useMemo(() => {
    ...
    const result: ArcDatum[] = SEVERITY_META
      .map(meta => {
        const val = counts[meta.key] || 0;
        const pct = val / tot;
        const sweep = pct * (360 - gap * SEVERITY_META.length);
        const arc: ArcDatum = {
          ...meta,
          value: val,
          pct,
    ...
    return { arcs: result, total: incidents.length };
  }, [incidents]);
  ...
  if (loading) {
    return (
      <Card className="bg-gradient-card border-border">
        <CardHeader><CardTitle className="flex items-center gap-2"><ShieldAlert className="w-5 h-5 text-primary" />Severity Distribution</CardTitle></CardHeader>
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="w-[200px] h-[200px] flex-shrink-0">
            <svg viewBox={`0 0 ${size} ${size}`} width="100%" height="100%">
              <defs>
                {SEVERITY_META.map(m => (
                  <filter key={m.key} id={`glow-sev-${m.key}`}>
              ...
              {arcs.map((arc) => {
                ...
                return (
                  <path
                    ...
                    filter={isHovered ? `url(#glow-sev-${arc.key})` : undefined}
          ...
          <div className="flex-1 min-w-0 pt-2 space-y-3">
            {SEVERITY_META.map(meta => {
              ...
              return (
                <div
                  key={meta.key}
                  className={`flex items-center gap-3 group cursor-pointer transition-all duration-200 rounded-md px-2 py-1.5 -mx-2 ${
